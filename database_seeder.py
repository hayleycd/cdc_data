...
def load_city_table():
	for raw in model.sqla_session.query(model.RawEntry).all():
		if raw.location in INVALID_CITIES_DICT.keys():
			break
		if not model.sqla_session.query(model.City).filter_by(name=raw.location).first():
	...
	for city in model.sqla_session.query(model.City).all():
		...
		city.lat = content["results"][0]['location']["lat"]
		city.lon = content["results"][0]['location']["lng"]
		model.sqla_session.commit()

def load_invalid_city_data():
	for each in INVALID_CITIES_DICT.keys():
		city = model.City(name = each, lat = INVALID_CITIES_DICT[each]['lat'], lon = INVALID_CITIES_DICT[each]['lon'])
		model.sqla_session.add(city)
	model.sqla_session.commit()

def load_case_in_city_for_year():
	for raw in model.sqla_session.query(model.RawEntry).all():
		city = model.sqla_session.query(model.City).filter_by(name=raw.location).first()
		if not model.sqla_session.query(model.CaseYearCity).filter_by(year=raw.year_diagnosed)\
				.filter_by(city=city.id).first():
			cases = model.CaseYearCity(city = city.id, year = raw.year_diagnosed, cases = raw.cases)
